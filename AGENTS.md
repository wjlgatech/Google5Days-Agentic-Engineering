...
- `tests/` — `pytest` wrappers that import each `modules/M*/exercise/*.py` and assert its `selftest()` returns 0 (auto-discovers new modules) + `test_fallback.py` (the webapp provider-chain order). Authoritative check is `scripts/check.sh` (O10); this is the dev-facing test view. Run: `pytest tests/ -q`.
- `scripts/new-module.py` — the **module generator** (Heighten): `python3 scripts/new-module.py M6-… "Title"` scaffolds a schema-conformant module that passes O4 (7 headings ×5 personas) + O10 (selftest) by construction. Declare a module, don't hand-author 9 files. Then wire it into `index.html` (O12) + `LEARNING-PATHS.md` (O13).
- `Makefile` (`make check` = gate + `pytest`) + `.github/workflows/ci.yml` — **CI** runs the full bar on every push/PR (Lengthen); no secrets needed (gate is deterministic).
- `paths/onboarding.md` — **cohort mode** (Widen): the shared team track; lessons carry an `author`; the gate is the team's CI.
- `webapp/guide.py` — the guide agent's **free-LLM fallback chain** (Gemini → Groq → NVIDIA NIM → OpenRouter): `active_providers(env)` picks configured providers in order, `route()` falls through on error. Set any of `GEMINI/GROQ/NVIDIA/OPENROUTER_API_KEY`. `tools/spec-to-green/mcp_server.py` exposes spec→green over **MCP** for any agent client.
